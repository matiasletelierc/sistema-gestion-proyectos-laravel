    ...
    <div class="container">
        ...
        <form action="{{ route('projects.update', $project->id) }}" method="POST" class="project-form">
            ...
            <div class="form-group">
                ...
                <input type="number" id="amount" name="amount" value="{{ old('amount', $project->amount) }}" step="0.01" min="0" required>
                @error('amount')
                    <span class="error">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-actions">
                ...
                <a href="{{ route('projects.show', $project->id) }}" class="btn btn-info">Ver Proyecto</a>
            </div>
        </form>
    </div>
</body>
